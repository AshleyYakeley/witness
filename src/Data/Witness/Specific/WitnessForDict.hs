...
newtype WitnessForDict f w =
    MkWitnessForDict [SomeFor f w]
    deriving (Semigroup, Monoid)

-- | An empty dictionary.
emptyWitnessForDict :: WitnessForDict f w
emptyWitnessForDict = mempty

-- | Look up the first value in the dictionary that matches the given witness.
witnessForDictLookup :: (TestEquality w) => w a -> WitnessForDict f w -> Maybe (f a)
...
witnessForDictModify :: (TestEquality w) => w a -> (f a -> f a) -> WitnessForDict f w -> WitnessForDict f w
...
witnessForDictReplace :: (TestEquality w) => w a -> f a -> WitnessForDict f w -> WitnessForDict f w
...
witnessForDictAdd :: w a -> f a -> WitnessForDict f w -> WitnessForDict f w
...
witnessForDictSingle :: w a -> f a -> WitnessForDict f w
witnessForDictSingle wit fa = MkWitnessForDict $ pure $ MkSomeFor wit fa

witnessForDictFold :: Monoid m => WitnessForDict f w -> (forall a. w a -> f a -> m) -> m
...
witnessForDictRemove :: (TestEquality w) => w a -> WitnessForDict f w -> WitnessForDict f w
...
witnessForDictFromList :: [SomeFor f w] -> WitnessForDict f w
witnessForDictFromList = MkWitnessForDict

witnessForDictMapM :: Applicative m => (forall a. f a -> m (g a)) -> WitnessForDict f w -> m (WitnessForDict g w)
